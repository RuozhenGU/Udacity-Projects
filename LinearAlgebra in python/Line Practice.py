...
class Line(object):
    ...
    def set_basepoint(self):
        try:
            ...
            basepoint_coords[initial_index] = c/initial_coefficient   # -c
    ...
    def __eq__(self, ell): ##check whether two lines are the same
        

        if self.normal_vector.is_zero(): #if cond to check if any of two lines is zero vector
    ...
    def intersection_with(self, ell):
        try:
            ...
            if self == ell: #if two lines equal, we use == because we define fun __eq__ before 
                return self
            else:       # two lines do not equal and they must be parallel
                return None
...
ell1=Line(normal_vector=Vector(['7.204','3.182']), constant_term = '8.69')
ell2=Line(normal_vector=Vector(['8.172','4.11']), constant_term = '9.883')
